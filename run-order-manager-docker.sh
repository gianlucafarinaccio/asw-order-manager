...
docker run -d --network=order-manager-net -e "SPRING_PROFILES_ACTIVE=order-service" -e "ASW_ORDERMANAGER_ORDERSERVICE_INSTANCENAME=os1" --name=order-service om-order-service
echo

echo Starting product-service in a Docker Container
docker run -d --network=order-manager-net -e "SPRING_PROFILES_ACTIVE=product-service" -e "ASW_ORDERMANAGER_PRODUCTSERVICE_INSTANCENAME=ps1" --name=product-service om-product-service
echo

echo Starting order-validation-service in a Docker Container
docker run -d --network=order-manager-net -e "SPRING_PROFILES_ACTIVE=order-validation-service" -e "ASW_ORDERMANAGER_ORDERVALIDATIONSERVICE_INSTANCENAME=ov1" --name=order-validation-service om-order-validation-service
echo
...
docker run -d --network=order-manager-net -p 8080:8080 --name=apigateway om-apigateway